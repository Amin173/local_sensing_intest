...
class AnalyticModel:
    ...
    def transform_broadcaster(self, bot_id):
        ...
        q = tf_conversions.transformations.quaternion_from_euler(0, 0, (
            ...
            bot_number] - self.data_dict[self.origin_tag][2])) * np.pi / 180)
    ...
    def update_aprilt_state_values(self, data):
        ...
        # rospy.loginfo(self.data_dict)
        if not self.all_imus_initiated:
            # rospy.loginfo("Using apriltag heading angles")
            for i in range(self.num_of_bots):
...
if __name__ == '__main__':
    ...
    rospy.Subscriber("imu_quaternions", String, broadcaster.update_imu_quaternions)
    ...
    while not rospy.is_shutdown():
        try:
            ...
            if broadcaster.all_imus_initiated:
                ...
                analyt_input = np.array(broadcaster.heading_angles_rel).reshape((1, 12))
                rel_poses = broadcaster.analyt_model(analyt_input)
                broadcaster.x_rel = rel_poses[:, 0] / 100
                broadcaster.y_rel = - rel_poses[:, 1] / 100
                for i in range(1, broadcaster.num_of_bots):
                    broadcaster.transform_broadcaster(broadcaster.key(i))
                pose_offset = Pose()
                pose_offset.position.x = np.mean(broadcaster.x_rel) - broadcaster.x_rel[0]
                pose_offset.position.y = np.mean(broadcaster.y_rel) - broadcaster.y_rel[0]
                pose_offset.position.z = broadcaster.heading_angles_rel[0]
                broadcaster.pose_offset_pub.publish(pose_offset)
